...
class ProductSchema(ma.Schema):
    ...
    _links = ma.Hyperlinks(
        {"self": ma.URLFor("product", product_id="<product_id>"),
         "collection": ma.URLFor("products"),
         "environments": ma.URLFor("environments", product_id="<product_id>")}
    )
...
class EnvironmentSchema(ma.Schema):
    ...
    _links = ma.Hyperlinks(
        {"self": ma.URLFor("environment", environment_id="<environment_id>", product_id="<product_id>"),
         "collection": ma.URLFor("environments", product_id="<product_id>")}
    )
...
def environment(environment_id, product_id):
    filtered_products = [p for p in DATA.keys() if str(p.product_id) == product_id]
    if filtered_products:
        single_product = filtered_products[0]
    else:
